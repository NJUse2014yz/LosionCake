...
public class GoodController {
    ...
    public Message insertGood(HttpServletRequest request, HttpServletResponse response){
        ...
        int result= DAOManager.goodDao.addGood(good);
        if(result!=-1) {
            message.setMsg("添加成功");
    ...
    public Message deleteGood(@RequestParam("goodId") String goodId){
        ...
        int result=DAOManager.goodDao.deleteGood(Integer.parseInt(goodId));
        if(result==-1) {
    ...
    public Message updateGood(HttpServletRequest request, HttpServletResponse response){
        ...
        int result=DAOManager.goodDao.updateGood(good);
        if(result==-1) {
            message.setMsg("更新失败");
            message.setResult("fail");
            return message;
    ...
    public Message getGood(@RequestParam("goodId") String goodId){
        ...
        Good result=DAOManager.goodDao.getGood(Integer.parseInt(goodId));
        if(result==null){
            message.setMsg("商品不存在");
            message.setResult("fail");
            return message;
    ...
    public Message getGoodList(){
        ...
        List<Good> result=DAOManager.goodDao.getGood();
        if(result==null) {
            message.setMsg("没有商品");
            message.setResult("fail");
            return message;
